    ...
    <div class="min-h-screen text-white mb-12">
        <main class="flex-grow ">
            ...
            <section class="relative mb-20">
                <div id="moviesHeader" class="sticky top-0 mt-[70px]  z-10 bg-transparent px-8 md:px-16 transition-colors duration-300">
                ...
                <div class="relative w-full h-[800px] overflow-hidden shadow-2xl">
                    <!-- Poster -->
                    <img id="heroPoster" src="{{ $poster }}" alt="{{ $featured->title }}" loading="lazy" class="w-full h-full object-cover absolute inset-0 transition-opacity duration-700 opacity-100" fetchpriority="high">

                    <!-- Video -->
                    @if($featured->file_path)
                    <video id="heroVideo" class="w-full h-full object-cover absolute inset-0 opacity-0 transition-opacity duration-700" muted playsinline preload="auto" poster="{{ $poster }}">
                    ...
                    <div class="absolute bottom-75 left-15 z-10 transition-opacity duration-700 opacity-100">
                        <img src="{{ asset('storage/' . $featured->title_poster) }}" alt="{{ $featured->title }}" loading="lazy" id="titlePoster" class="mb-4 w-[500px] h-auto object-contain transition-all duration-700">
                        ...
                        <div class="flex space-x-4">
                            <button class="bg-[#FFFFFF] text-black font-bold w-[126px] h-[46px] rounded-md! justify-center hover:bg-[#C3C1C1] transition duration-300 flex items-center space-x-2">
                            ...
                            <!-- hero banner -->
                            <button id="moreInfoBtn" class=" bg-[#6d6d6e] mr-0.5 text-[#ffffff]! opacity-[0.7] font-bold w-[172px] h-[46px] rounded-md! justify-center hover:bg-[#403A36] transition duration-300 flex items-center space-x-2" data-video-id="{{ $featured->id }}" data-title="{{ $featured->title }}" data-description="{{ $featured->description }}" data-duration="{{ $featured->duration ?? '1h 52m' }}" data-poster="{{ $poster }}" data-file="{{ $featured->file_path ? asset('storage/' . $featured->file_path) : '' }}" data-title-poster="{{ $featured->title_poster ? asset('storage/' . $featured->title_poster) : '' }}" data-genres="{{ $additionalSection['metadata']['genre'] ?? 'Drama • Action' }}">
                                <svg viewBox="0 0 24 24" width="24" height="24" data-icon="CircleIMedium" data-icon-id=":r25:" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" role="img">
