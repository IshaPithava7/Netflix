...
<div class="flex justify-center items-center px-4">
    ...
    <div class="bg-black/80 p-10 rounded max-w-md w-full z-10">
        ...
        <form class="flex flex-col gap-4 w-full max-w-md mx-auto login-form" action="{{ route('login') }}"
            method="POST">
            @csrf
            <!-- Email -->
            <input type="email" name="email" value="{{ old('email', $email)}}" placeholder="Email address" required
